...
if __name__ == '__main__':
    ...
    parser.add_argument('-cohort', '--cohort', help='(OPTIONAL) Which trained model to use: TLE or MCI (default: TLE)',
                        type=str, choices=['TLE', 'MCI'], default='TLE')
    ...
    # weights for the chosen cohort: checkpoints/FlowGAN_<cohort>/
    cohort_checkpoints_dir = os.path.abspath(os.path.join('checkpoints', f'FlowGAN_{args.cohort}'))

    if not os.path.isdir(cohort_checkpoints_dir):
        raise SystemExit(
            f'Could not find the {args.cohort} model weights at {cohort_checkpoints_dir}\n'
            'Download checkpoints.tar.gz (see the README) and unpack it inside the code '
            'directory, so that code/checkpoints/FlowGAN_TLE/ and code/checkpoints/FlowGAN_MCI/ exist.')
    ...
    if ensemble:
        ...
        FlowGAN_inference_ensemble.test_all_folds(
            model_dir=os.path.join(cohort_checkpoints_dir, 'FlowGAN_ensemble'),
        ...
        FlowGAN_inference.test_model(
            model_name='FlowGAN',
            data_source=os.path.abspath(args.output_dir),
            checkpoints_dir=cohort_checkpoints_dir,
            output_dir=os.path.abspath(args.output_dir),
            pytorch_CycleGAN_and_pix2pix_dir=os.path.abspath('pytorch-CycleGAN-and-pix2pix/'),
            direction='BtoA'
        )
