...
# average pet for single subject
def average_pet(subject, input_dir, output_dir, n_splits=12):
...
def average_in_series(full_subject_list, input_dir, output_dir, n_splits=12):
    ...
    for sub in full_subject_list:
        ...
        average_pet(sub, input_dir, output_dir, n_splits)

# iterate in parallel
def average_in_parallel(full_subject_list, input_dir, output_dir, n_splits=12):
    ...
    with multiprocessing.Pool(processes=max_processes) as pool:
        pool.starmap(average_pet, list_of_arguments)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Average smoothed outputs across all folds in ensemble')
    ...
    parser.add_argument('-data','--data',
                        help='Directory where coregistered smoothed outputs from each fold are stored',
                        required=True,
                        )
    
    # output directory
    parser.add_argument('-output_dir','--output_dir',
                        help='Directory to output the averaged smoothed outputs',
                        required=True,
                        )
