...
# iterate over list of subjects for a single fold
def coregister_all_subjects_single_fold(full_subject_list, recon_dir, out_dir):
...
# coregister all subjects for each fold
def coregister_all_subjects(full_subject_list, recon_dir, out_dir, n_splits=12):
    # get list of folds
    folds = [f'fold_{fold}' for fold in range(0, n_splits)]

    for fold in folds:
        print(f'Coregistering subjects for {fold}')

        recon_dir_fold = os.path.join(recon_dir, fold, 'recon_niftis')

        out_dir_fold = os.path.join(out_dir, fold, 'recon_niftis_coregistered')

        coregister_all_subjects_single_fold(full_subject_list, recon_dir_fold, out_dir_fold)

    print('Finished coregistering subjects')
